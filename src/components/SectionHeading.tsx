...
export default function SectionHeading({ title, subtitle, light }: Props) {
  ...
  return (
    <div ref={ref} className={`text-center mb-12 md:mb-16 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
      <h2 className={`font-heading text-3xl md:text-4xl lg:text-5xl mb-4 ${light ? "text-white" : "text-foreground"}`}>
        {title}
      </h2>
      ...
      {subtitle && (
        <p className={`mt-4 text-base md:text-lg max-w-2xl mx-auto ${light ? "text-gray-300" : "text-muted-foreground"}`}>
          {subtitle}
        </p>
      )}
    </div>
